...
/**
 * @file plugins/generic/viewerMultimedia/viewerMultimediaPlugin.inc.php
 *
 * Copyright (c) 2014-2020 Simon Fraser University
 * Copyright (c) 2003-2020 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class viewerMultimediaPlugin
 * @ingroup plugins_generic_viewerMultimedia
 *
 * @brief Class for viewerMultimedia plugin
 */

import('lib.pkp.classes.plugins.GenericPlugin');

class viewerMultimediaPlugin extends GenericPlugin {
	/**
	 * @see Plugin::register()
	 */
	...
	function getDisplayName() {
		return __('plugins.generic.viewerMultimedia.displayName');
	}
	...
	function getDescription() {
		return __('plugins.generic.viewerMultimedia.description');
	}
	...
	function articleDownloadCallback($hookName, $args) {
		...
		if ($galley && $galley->getFileType() == 'audio/mpeg' && $galley->getFileId() == $fileId) {
			if (!HookRegistry::call('viewerMultimediaPlugin::articleDownload', array($article,  &$galley, &$fileId))) {
				echo $this->_getHTMLContents($request, $galley);
				$returner = true;
				HookRegistry::call('viewerMultimediaPlugin::articleDownloadFinished', array(&$returner));
			}
			return true;
		}
